                ...
                <div class="d-grid gap-2 d-flex justify-content-start">
                    @guest
                        <button type="button" onclick="location.href='{{ route('login') }}'"
                            class="btn btn-outline-dark btn-lg px-4 me-2">Login</button>
                        <button type="button" onclick="location.href='{{ route('register') }}'"
                            class="btn btn-dark btn-lg px-4">Registrati</button>
                    @endguest

                    @auth
                        <button type="button"
                            onclick="location.href='{{ route('user', ['username' => Auth::user()->username]) }}'"
                            class="btn btn-outline-dark btn-lg px-4 me-2">Vai al tuo profilo</button>
                    @endauth
                </div>

                <div class="my-5">
                    <button type="button" onclick="location.href='https://github.com/mrgian/nexum'"
                        class="btn btn-outline-dark px-4 me-2">Leggi la documentazione</button>
                    <button type="button" onclick="location.href='https://github.com/mrgian/nexum'"
                        class="btn btn-outline-dark px-4 me-2">Vai al codice sorgente</button>
                </div>
            </div>
            <div class="col-6">
                <img src="{{ asset('assets/social.png') }}" class="d-block mx-lg-auto img-fluid" loading="lazy"
